...
A *newly* added PATH directory is not visible to the running Gemini CLI session — but the gate does
...
- **Windows** — install **Git for Windows** (https://git-scm.com/download/win) for `sh`, and
  **Python 3** from https://www.python.org/downloads/ (use the python.org installer, **not** the
  ...
  gate nor Gemini CLI's own Bash tool can run (Gemini falls back to PowerShell, and the
...
If the deny says cx is installed but **missing `cx mcp bridge` / `cx hooks gemini-*`**, the installed
...
```bash
# macOS/Linux (set where Gemini CLI is launched, e.g. your shell profile):
export CX_BINARY="/opt/checkmarx/cx"
# Windows (User environment variable, so the Gemini CLI process inherits it):
