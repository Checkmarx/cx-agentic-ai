#!/usr/bin/env bash
# Table-driven tests for Gemini cx-asset-resolver.sh (versioned release assets).
# No network. Run: bash tests/scripts/test_cx_asset_resolver_gemini.sh
set -uo pipefail
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=/dev/null
source "$DIR/../../scripts/cx-asset-resolver.sh"
set +e
