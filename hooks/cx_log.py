...
_EVENTS = {
    ...
    "mcp_connect": {
        ...
        # exact, on-disk reason instead of only Gemini's generic "-32000 / failed to reconnect".
...
def _assistant():
    ...
    config (hooks.json sets CX_ASSISTANT=claude or CX_ASSISTANT=gemini-cli) so each client writes
    to its own log subdirectory and every log entry carries the correct assistant label.
    Falls back to 'claude' when unset so existing Claude Code deployments that don't yet
    pass CX_ASSISTANT keep working."""
...
def _plugin_version():
    ...
    #   1. plugin.json at the plugin root (shared, written by both Claude Code and Gemini CLI)
    #   2. .claude-plugin/plugin.json (Claude Code legacy location)
    #   3. .plugin/plugin.json (Gemini CLI location)
    # Reading any one that has a parseable version is sufficient.
    _base = os.path.dirname(os.path.abspath(__file__))
    for rel in (
        os.path.join("..", "gemini-extension.json"),
