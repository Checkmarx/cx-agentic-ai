...
#   - …pre-tool-use / …before-tool (a Checkmarx MCP call) -> emit a deny JSON + exit 2 (Claude) or
#     exit 0 + decision:"deny" (Gemini) so the tool call is BLOCKED, unscanned.
#   - …pre-file-write / …before-file-tool -> exit 0, DEFERRING to stage 1 (cx_check.sh ->
#     cx_check.py), which has already decided this same call and correctly: it denies a scannable file
#     when cx is absent and allows an unscannable one. Denying again here cannot make a scannable write
#     safer (verdicts merge most-restrictive-wins, so stage 1's deny already stands) and DOES block the
#     unscannable writes stage 1 just allowed.
#   - Advisory lifecycle hooks (…stop / …idle / …prompt-submit / …before-agent / …after-agent) ->
#     exit 0 (non-blocking by design; a fail-closed prompt-submit would deadlock the user before install).
...
# Whether this invocation IS the blocking scan decision — the same substring match used below in
# the cx-unresolved branch, kept in lockstep with it. Matches Claude's route names
# (…pre-tool-use / …pre-file-write) and Gemini CLI's real dispatch route from ast-cx-hooks' route
# catalog (…before-file-tool). Advisory lifecycle hooks are not security decisions and stay on the
# plain exec fast path.
case "${1:-} ${2:-}" in
    *pre-tool-use* | *pre-file-write* | *before-tool* | *before-file-tool*) _CXRUN_SCAN=1 ;;
    *)                                                      _CXRUN_SCAN=0 ;;
esac

# Whether this invocation IS the MCP bridge spawn declared in the extension manifest — the
# exact-match (not a wildcard like the scan patterns above) so nothing else can accidentally take
# the guarded path.
case "${1:-} ${2:-}" in
...
if [ -n "$CX_RESOLVED" ]; then
    if [ "$_CXRUN_SCAN" = 1 ]; then
        ...
        # Matches BOTH client shapes: Claude Code's "permissionDecision":"deny" and Gemini CLI's
        # flat "decision":"deny" (Gemini has no permissionDecision/additionalContext fields at all).
        case "$_CXRUN_OUTPUT" in
            *'"permissionDecision":"deny"'* | *'"decision":"deny"'* | *'"decision": "deny"'*)
        ...
        # carries valid deny JSON. Normalize structured denies to exit 0; decision:"deny" blocks.
        if [ "${CX_ASSISTANT:-}" = "gemini-cli" ] && [ "$_CXRUN_STATUS" -ne 0 ]; then
            case "$_CXRUN_OUTPUT" in
    ...
    if [ "$_CXRUN_MCP" = 1 ]; then
        ...
        if [ "$_CXRUN_MCP_RESULT" = denied ]; then
            ...
            # the reason goes to stderr, surfaced to the Gemini agent as the MCP connection error.
            # AND to checkmarx-devassist.jsonl above, so a connect failure always has an exact cause on disk.
            #
            # For Gemini: this message is informational only. Enforcement happens via the preToolUse
...
case "${1:-} ${2:-}" in
    *pre-tool-use* | *pre-file-write* | *before-tool* | *before-file-tool*)
        ...
        # A FILE WRITE defers to stage 1 here; only an MCP call (pre-tool-use / before-tool) denies below.
        #
        # There is still NO file-type rule in this file — that rule lives ONLY in cx_check.py's
        # _is_scannable_file. With cx unresolvable, stage 1 (cx_check.sh -> cx_check.py) has ALREADY
        # decided this exact call: it denies a scannable file when cx is absent and allows an unscannable
        # one. Denying again here blocks the unscannable writes stage 1 just allowed.
        case "${1:-} ${2:-}" in
            *pre-file-write* | *before-file-tool*) exit 0 ;;
        esac
        # Detect Gemini CLI context from the route name (ast-cx-hooks' route catalog names Gemini's
        # BeforeTool routes "gemini-before-tool"/"gemini-before-file-tool"). Gemini CLI reads flat
        # JSON with decision/reason/systemMessage at the top level and treats exit 0 as hook success
...
JSON
        ...
        # Gemini: exit 0 (deny JSON blocks). Claude: exit 2 (required deny signal).
        case "$*" in
            *gemini-before*) exit 0 ;;
            *) exit 2 ;;
        esac
        ;;
    *stop* | *idle* | *prompt* | *before-agent* | *after-agent*)
        # Advisory lifecycle hook (stop / idle / user-prompt-submit, or Gemini's before-agent/
        # after-agent) — stay non-blocking when cx is genuinely absent; blocking a prompt-submit
        # here would deadlock the user before install.
        exit 0
        ;;
    *)
        if [ "$_CXRUN_MCP" = 1 ]; then
