...
`BeforeTool` hooks run on every `WriteFile`, `write_file`, `write_.*`, `replace`, `run_shell_command`,
and `mcp_.*` tool call without skill activation:

- **Gate** (`cx_check`) — proves `cx` is installed, capable, and authenticated.
- **Scanner** (`cx hooks gemini-before-file-tool`) — scans proposed file content (SAST for source,
  SCA for manifests like `package.json`).
...
remediation — or when a hook deny message tells you to.
