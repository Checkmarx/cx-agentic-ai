...
case "${1:-} ${2:-}" in
    *pre-tool-use* | *pre-file-write* | *before-tool* | *before-file-tool*)
        ...
        case "$*" in
            *gemini-before*)
                printf '{"decision":"deny","reason":"The Checkmarx security scanner could not run: cx CLI not found (not in CX_BINARY, canonical store, or PATH). This operation is BLOCKED fail-closed. Run /checkmarx-cli-setup to install cx, then retry.","systemMessage":"Checkmarx CLI (cx) is not installed — this action was blocked. Run /checkmarx-cli-setup or the bootstrap install command."}\n'
                ;;
            *)
                cat <<'JSON'
