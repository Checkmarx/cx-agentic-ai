...
# Invoked by hooks.json (Gemini) as:     sh "${extensionPath}/hooks/cx_check.sh" --gemini-cli
#
...
#   Gemini CLI:   deny = exit 0 + {"decision":"deny",...,"systemMessage":...} on stdout.
#                 Non-zero exit marks the hook as FAILED (generic warning) even when JSON is valid.
    ...
    case "$*" in
        *--gemini-cli*)
            printf '{"decision":"deny","reason":"%s %s","systemMessage":"%s"}\n' \
                "$_NO_PY_REASON" "$_NO_PY_CONTEXT" "$_NO_PY_REASON"
            exit 0
            ;;
        *)
