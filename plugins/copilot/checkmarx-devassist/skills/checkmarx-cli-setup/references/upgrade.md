...
- The **running MCP bridge keeps the old code until `/restart`** re-spawns it against the
...
Copilot already spawned keeps running the OLD binary until it is explicitly re-spawned via
`/restart`. Run:
