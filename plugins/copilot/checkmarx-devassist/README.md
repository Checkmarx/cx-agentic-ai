...
the gate's `cx auth login` recovery command and the `checkmarx-cli-setup` skill skips the URL/tenant question.
...
claude plugin install checkmarx-devassist@checkmarx-devassist-marketplace
```

Verify:

```bash
claude plugin list      # checkmarx-devassist@checkmarx-devassist-marketplace  ✔ enabled
```

**Direct plugin-dir** (quick local testing):

```bash
claude --plugin-dir /path/to/cxone-scanners/plugins/checkmarx-devassist
```
...
`~/.checkmarx/agent-logs/<assistant>/checkmarx-devassist.jsonl` — both the stage-1 readiness gate's own
