...
though a running Cursor froze its PATH at startup and cannot see a newly-persisted PATH entry.
...
precedence as the gate) and starts when Cursor loads the plugin's MCP servers, so it activates after
**one Developer: Reload Window** — no full restart, and no need to hand-edit `mcp.json` or put cx on
PATH for it.
