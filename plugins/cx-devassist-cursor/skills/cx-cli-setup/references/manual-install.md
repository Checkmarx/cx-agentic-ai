...
**Git for Windows is mandatory.** The gate launches through `sh`, and a reliable shell tool needs
Git Bash too, so install it first from https://git-scm.com/download/win. Without it the gate cannot
run at all.
...
folder that happens to be on PATH.

Run the block below **in Git Bash** (not PowerShell or cmd — it uses POSIX syntax and `$LOCALAPPDATA`
with forward slashes). Git Bash is a prerequisite on Windows anyway, per the note above. A PowerShell
equivalent is in `references/shells.md`.
...
**Developer: Reload Window** (see `references/mcp.md`).
