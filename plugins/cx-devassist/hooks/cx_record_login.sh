#!/bin/sh
# cx_record_login.sh — the ONLY hook the plugin puts on Bash/PowerShell, and it is an OBSERVER, not a
...
#   exit 0). The readiness gate lives only on the file-write and MCP matchers now, so a broken cx can
#   no longer stop a shell command.
#
# COST
...
# Only Bash/PowerShell payloads are worth handing to Python; a file write can never be a login.
case "$INPUT" in
...
# Unconditional: never let this observer's outcome reach Claude Code as a blocking signal.
exit 0
