...
#   A developer's own interactive login DOES leave the values on disk — see the note in cx_check.py's
#   cx_record_login docstring about reading them as an additional source.
#
# WHY THIS CANNOT REINTRODUCE "EVERY COMMAND IS BLOCKED"
#   Copilot CLI denies via `permissionDecision:deny` JSON (exit 0). This script emits NO JSON and
#   exits 0 on EVERY path — including a missing Python, an unreadable state dir, and any crash inside
#   the recorder (cx_check.py's record-login mode swallows exceptions and forces exit 0). The
#   readiness gate lives only on create/edit now, so a broken cx can no longer stop a shell command.
#
# COST
...
#
# Scoped to the COMMAND, not the whole payload. The payload also carries `cwd`, `sessionId` and a
...
# Only shell-command payloads are worth handing to Python; a file create/edit can never be a login.
# Copilot CLI names: powershell / bash / shell (and the synthetic `command` used in tests). Also
# recognises Claude-style Bash/PowerShell and Copilot's toolName / toolCalls `name` spellings.
case "$INPUT" in
...
# Unconditional: never let this observer's outcome reach Copilot CLI as a blocking signal.
exit 0
