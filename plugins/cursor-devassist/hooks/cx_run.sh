    ...
    if [ "$_CXRUN_SCAN" = 1 ]; then
        ...
        if [ "$_CXRUN_DECISION" = deny ] && [ "$_CXRUN_REASON" = error_during_block ]; then
            case "$_CXRUN_OUTPUT" in
                ...
                *)
                    _CXRUN_OUTPUT='{"permission":"deny","user_message":"The Checkmarx security scanner did not return a result (it may have timed out or crashed). This operation is BLOCKED fail-closed.","agent_message":"CHECKMARX_HOOK_DENY — The Checkmarx security scan did not complete (timed out or crashed) for this command. This is usually transient. Wait a few seconds and retry the exact same command once. If it fails the same way again, stop and report it — do not re-wrap it in bash -c, cmd /c, or backtick-escaping.","additional_context":"CHECKMARX_HOOK_DENY — The Checkmarx security scan did not complete (timed out or crashed) for this command. This is usually transient. Wait a few seconds and retry the exact same command once. If it fails the same way again, stop and report it — do not re-wrap it in bash -c, cmd /c, or backtick-escaping."}'
                    _CXRUN_STATUS=2
                    ;;
            esac
        fi
