...
_EVENTS = {
    ...
    "scan_decision": {
        # The stage-2 native `cx hooks cursor-*` scanner's own allow/deny — distinct from
    ...
    "mcp_connect": {
        ...
        # Developer: Reload Window, MCP reconnect) — success or denial — so a connect failure always
        # has an exact, on-disk reason instead of only a generic "-32000 / failed to reconnect".
...
_MCP_CONNECT_MESSAGES = {
    ...
    "below": "cx v{have} is below the required v{min} — mcp bridge blocked; run scripts/cx-bootstrap.sh upgrade.",
    "incapable": ("cx v{have} is missing the 'mcp bridge' subcommand (capability-incomplete build) — "
                  "mcp bridge blocked; run scripts/cx-bootstrap.sh upgrade."),
    ...
    "cx_absent": ("cx CLI could not be resolved via CX_BINARY, the canonical store, or PATH — mcp "
                  "bridge blocked; run scripts/cx-bootstrap.sh install."),
...
def _assistant():
    return _token(os.environ.get("CX_ASSISTANT", "")) or "cursor"
...
def _plugin_version():
    ...
    try:
        path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "..", ".cursor-plugin", "plugin.json"
        )
        with open(path) as f:
            data = json.load(f)
