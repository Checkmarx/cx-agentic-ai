...
A *newly* added PATH directory is not visible to the running Cursor session — but the gate does
...
Run these as two separate bare commands (never chained). Bare `cx …` is identical in every shell; if
cx is not yet on PATH, prefix the canonical absolute path in your shell's form — see
...
- **Windows** — install **Git for Windows** (https://git-scm.com/download/win) for `sh`, and
  **Python 3** from https://www.python.org/downloads/ (use the python.org installer, **not** the
  ...
  gate nor Cursor's own shell tool can run reliably.
...
If the deny says cx is installed but **missing `cx mcp bridge` / `cx hooks cursor-*`**, the installed
...
```bash
# bash / sh — macOS/Linux (set where Cursor is launched, e.g. your shell profile):
export CX_BINARY="/opt/checkmarx/cx"
```

```powershell
# PowerShell — Windows (User environment variable, so the Cursor process inherits it):
[Environment]::SetEnvironmentVariable('CX_BINARY', 'C:\Tools\Checkmarx\cx.exe', 'User')
```

```bat
:: cmd.exe — Windows (equivalent to the PowerShell line above):
setx CX_BINARY "C:\Tools\Checkmarx\cx.exe"
```
...
- **The stage-2 scanner and the remediation MCP honor it too** — both run through `hooks/cx_run.sh`,
  ...
  or restart needed); it activates after one **Developer: Reload Window**. Do not hand-edit `mcp.json`.
- `CX_BINARY` must be set in the environment Cursor is launched with (the hooks inherit it);
...
Cursor launches with if hooks should always use Podman.
