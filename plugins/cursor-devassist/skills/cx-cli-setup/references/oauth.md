...
**Question 2 — ask in a plain chat message**: the tenant is org-specific free text with no preset
options, so collect both in a single reply rather than a multi-step choice:
...
The deny message renders that command **for each shell already**, with the detected one first — copy
its matching line verbatim. If you are composing it yourself, use the form for **your** shell (full
table in `references/shells.md`); the three differences are the `&` call operator, the path separator,
and the null device:

```bash
# bash / sh (macOS, Linux, Git Bash) — forward slashes, /dev/null
"$HOME/.checkmarx/bin/cx" auth login --base-auth-uri <URL> --tenant <tenant> 1>/dev/null
```

```powershell
# PowerShell (Cursor's default shell on Windows) — & call operator REQUIRED, $null
& "$env:LOCALAPPDATA\Checkmarx\cx\cx.exe" auth login --base-auth-uri <URL> --tenant <tenant> 1>$null
```

```bat
:: cmd.exe — backslashes, NUL
"%LOCALAPPDATA%\Checkmarx\cx\cx.exe" auth login --base-auth-uri <URL> --tenant <tenant> 1>NUL
```

Without the `&`, PowerShell treats the quoted path as a plain string and **prints it instead of
logging in** — the single most common way this step silently does nothing on Windows.

**Run this YOURSELF with the shell tool — do NOT hand it to the developer to type.** This is the
OAuth path: the agent runs the resolved login command itself (unlike an API key, which the
...
hands you, in **any** of the four shells' spellings (PowerShell call operator, `%VAR%` / `$env:VAR` /
`$VAR` references, single- or double-quoted paths, `cmd /c` and `bash -c` wrappers) — through even
while it is blocking every other action (that is exactly why the carve-out exists), so it will not be
blocked here. The browser opens automatically on the developer's machine;
they only complete the login there. If an earlier shell call was denied by the gate, that does NOT
mean the resolved `cx auth login` will be — the recovery commands are allowed; run it.
