...
# Synced fail-closed fallback with scripts/cx-release-tag (search: CX_RELEASE_TAG).
...
# ~/.checkmarx/agent-logs/cursor (or $CX_LOG_DIR), NOT the OS temp dir. Clearing it after a
# place lets the next hook fire re-probe the just-installed/upgraded cx immediately.
AGENT_LOG_DIR="${CX_LOG_DIR:-$HOME/.checkmarx/agent-logs/cursor}"
...
detect_os_arch() {
    ...
    ASSET="$(resolve_cx_asset "$(uname -s)" "$(uname -m)" "$tag" 2>/dev/null)" \
    ...
    # ast-cli_<tag>_<os>_<arch>.<ext>, so they always agree with the resolver.
    case "$ASSET" in
        *_linux_*)   OS="linux" ;;
        *_darwin_*)  OS="darwin" ;;
        *_windows_*) OS="windows" ;;
        *) die "could not derive OS from asset name: $ASSET" ;;
    esac
    case "$ASSET" in
        *.tar.gz) EXT="tar.gz" ;;
        *.zip)    EXT="zip" ;;
...
download_and_extract() {
    # $1 = the pinned release tag resolved ONCE by the caller. Downloading from that tag URL (and
    # verifying against THAT tag's checksums) closes a TOCTOU window where the tag could change
    # between the download and the checksum fetch, making us verify one release's archive against
    # another's checksums.
    local tag="${1:-}"
    local url staging archive bin
    [[ -n "$tag" ]] || die "internal error: release tag not resolved"
...
verify_checksum() {
    ...
    local archive="$1" tag="${2:-}" sums tag_url
    [[ -n "$tag" ]] || { _checksum_unavailable "release tag not resolved"; return $?; }
    sums="$(mktemp "${TMP_BASE%/}/cx-sums.XXXXXX")" || { _checksum_unavailable "could not create a temp file"; return $?; }
    tag_url="https://github.com/Checkmarx/ast-cli/releases/download/${tag}/ast-cli_${tag}_checksums.txt"
...
A capability-complete cx release is required (the public release may predate it; see scripts/cx-min-version)."
    ...
    "$cx_bin" hooks cursor-before-shell --help >/dev/null 2>&1 \
        || die "placed cx is missing 'cx hooks cursor-before-shell' — this build cannot run the \
Cursor security scanner. A capability-complete cx release is required."
    "$cx_bin" hooks cursor-after-file-edit --help >/dev/null 2>&1 \
        || die "placed cx is missing 'cx hooks cursor-after-file-edit' — this build cannot run the \
Cursor post-write scanner. A capability-complete cx release is required."
