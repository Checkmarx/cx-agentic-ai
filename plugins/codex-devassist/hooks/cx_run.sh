...
#   - `mcp bridge` — this is THE command .mcp.json / config.toml's [mcp_servers.Checkmarx] declares
#     as the MCP server itself, spawned by the host CLI outside the hook system entirely (no
#     PreToolUse gate runs first). A resolved cx that is below the minimum version or missing the
#     `mcp bridge` subcommand must NOT be exec'd blindly: it would die on cobra's "unknown command"
#     error before/during the JSON-RPC initialize handshake, which the host CLI would surface as a
#     generic, undiagnosable connection failure. So this
...
if [ -n "$CX_RESOLVED" ]; then
    ...
    if [ "$_CXRUN_MCP" = 1 ]; then
        ...
        if [ "$_CXRUN_MCP_RESULT" = denied ]; then
            ...
            #
            # This message is informational only. Enforcement happens via the PreToolUse hook
            # (cx_check.sh/cx_check.py) on the next Bash/apply_patch call — that hook blocks the
            # action and drives the agent to install/upgrade cx. The MCP reconnects after a
...
case "${1:-} ${2:-}" in
    *pre-tool-use* | *pre-file-write*)
        ...
        # and Codex CLI both read the nested hookSpecificOutput wrapper (Codex's PreToolUse
        # deny contract is confirmed identical to Claude's) — so this plugin's codex-* named
        # subcommands fall through to the same default/nested-JSON arm as Claude, needing no
        # separate case. Only the human-readable skill-invocation string differs by client.
        case "$*" in
            *copilot-cli*)
