...
**Git for Windows is mandatory.** The gate launches through `sh`, and Codex CLI needs Git Bash
for its own shell tool, so install it first from https://git-scm.com/download/win. Without it the
...
tool call** — no PATH placement, symlink, or restart needed. Only the remediation MCP needs a
Codex CLI session restart (see `references/mcp.md`).
