...
**Run this YOURSELF with Codex CLI's shell tool — do NOT hand it to the developer to run manually.**
This is the OAuth path: the agent runs the resolved login command itself (unlike an API key, which the
...
exists), so you never need to defer to the developer here. The browser opens automatically on the
developer's machine; they only complete the login there. If an earlier shell call was denied by the
gate, that does NOT mean the resolved `cx auth login` will be — the recovery commands are allowed;
run it.
