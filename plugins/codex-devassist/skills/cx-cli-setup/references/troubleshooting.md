...
the machine half-configured. If the developer already has a newer/internal capable build available
some other way, they can point the gate at it with `CX_BINARY` (below) instead of waiting on the
bootstrap.
