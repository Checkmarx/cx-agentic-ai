...
2. **Remediation** — User asks to fix KICS findings, or Codex needs to fix IaC misconfigurations detected by KICS.
...
- If the tool is **not available**, or a call **errors with "Transport closed"** (or any other
  connection/transport error): **STOP. Do NOT remediate by any other means.** Leave the finding
  **unfixed**, then follow the exact same MCP recovery flow as `cx-devassist-asca` (Flow 2 → Step 2) —
  including the "retry once before mentioning a restart" rule and the `/exit` + `codex resume`
  guidance for either failure mode. End without modifying code.
...
**This step is MANDATORY and is not satisfied by an ordinary prose completion message.** After Step 3
