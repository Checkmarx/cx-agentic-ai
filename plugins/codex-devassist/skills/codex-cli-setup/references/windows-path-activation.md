...
though a running Codex CLI session froze its PATH at startup and cannot see a newly-persisted PATH
entry.
...
precedence as the gate) and starts at session start, so it activates after **restarting the Codex CLI
session** so it re-reads `config.toml` and re-spawns the bridge — there is no confirmed hot-reload, and
no need to put cx on PATH for it.
